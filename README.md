...
This will create the image version related to the last tag (`git describe --abbrev=0`), e.g. `magneticio/vamp-runner:0.9.0`
...
```sh
$ docker run --net=host -e VAMP_RUNNER_API_URL=$VAMP_API_URL magneticio/vamp-runner:0.9.0
```

or just something like:

```sh
$ docker run --net=host -e VAMP_RUNNER_API_URL=http://192.168.99.100:8080/api/v1 magneticio/vamp-runner:0.9.0
```
...
clusters:
  vamp-runner:
    services:
      breed:
        name: vamp-runner:0.9.0
        deployable: magneticio/vamp-runner:0.9.0
        ports:
          port: 8088/http
