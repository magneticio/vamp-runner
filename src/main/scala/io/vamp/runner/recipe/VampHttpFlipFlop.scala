...
class VampHttpFlipFlop(implicit actorSystem: ActorSystem) extends Recipe {
  ...
  private val deployment = "deployments/http_flip_flop"
  ...
  private def deploy() = {
    apiPut(deployment, resource("httpFlipFlop/blueprint_1.0.yml")).flatMap { _ ⇒
      ...
      apiPut(deployment, resource("httpFlipFlop/blueprint_1.1.yml"))
    }
  }
  ...
  private def flipFlop() = {
    def flip() = {
      def switch(to: String) = {
        ...
        apiPut(deployment, resource(s"httpFlipFlop/blueprint_$to.yml")) flatMap { _ ⇒
          apiPut(deployment, resource(s"httpFlipFlop/set_100%_to_$to.yml")) flatMap { some ⇒
            if (delete)
              apiDelete(deployment, resource(s"httpFlipFlop/blueprint_$old.yml"))
